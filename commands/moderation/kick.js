const {
    ...
    async execute(interaction) {
      ...
      if (!member.kickable) {
        const embed = interaction.client.embed({
          title: `${interaction.client.emoji.no} | Not kickable`,
          description: `The user ${ban} is not kickable!`,
        ...
        collector.on("end", (collected) => {
          if (collected.size === 0) {
            interaction.editReply({
              content: `${interaction.client.emoji.no} | No response. Kick cancelled.`,
              embeds: [],
              components: [],
            });
          }
  ...
  async function handleConfirmBan(i, userToBan, reason, picture) {
    try {
      const dmEmbed = new EmbedBuilder()
        .setColor("#ff5555")
        .setTitle(`${i.client.emoji.ban} | You've been kicked from ${i.guild.name}`)
        .setThumbnail(i.guild.iconURL())
        .addFields(
          { name: `${i.client.emoji.moderator} Kicked by`, value: i.user.tag },
      ...
      const banConfirmationEmbed = new EmbedBuilder()
        ...
        .setDescription(`**${userToBan.tag}** has been kicked from the server.`)
      ...
      await i.guild.members.kick(userToBan, {
      ...
      const errorEmbed = new EmbedBuilder()
        .setColor("#ff0000")
        .setTitle(`${i.client.emoji.no} Kick Failed`)
        .setDescription(`Failed to kick **${userToBan.tag}** from the server.`)
        .setTimestamp()
        .setFooter({
  ...
  async function handleCancelBan(i) {
    const cancelEmbed = new EmbedBuilder()
      .setColor("#ffff00")
      .setTitle(`${i.client.emoji.yes} Kick Cancelled`)
